...
export async function POST(request: NextRequest) {
  try {
    ...
    const rawOldPrice = body.oldPrice !== undefined && body.oldPrice !== null && body.oldPrice !== "" ? Number(body.oldPrice) : null;

    if (!title) return NextResponse.json({ error: "Title is required." }, { status: 400 });
    if (!courseDescription) return NextResponse.json({ error: "Course description is required." }, { status: 400 });
